...
FixPATH_DYNAMICS::FixPATH_DYNAMICS(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg),id_pe(NULL),pe(NULL)
{
  ...
  dt_path   = force->numeric(FLERR,arg[3]);
  damp_path = force->numeric(FLERR,arg[4]);
  ...
  if (narg>iarg){
	  if (strcmp(arg[iarg],"langevin") == 0) {
	    ...
	    run = LANGEVIN;
	    target_temp = force->numeric(FLERR,arg[iarg+1]);
	    lange_tau   = force->numeric(FLERR,arg[iarg+2]);
	    ...
	    run = NHC;
	    target_temp = force->numeric(FLERR,arg[iarg+1]);
	    nhc_nchain = force->inumeric(FLERR,arg[iarg+2]);
	    if ( target_temp <= 0.0 ) error->universe_all(FLERR,"Fix path_dynamics temp must be > 0");
	    ...
	    run = DAMPED;
	    lange_tau = force->numeric(FLERR,arg[iarg+1]);
	    if (lange_tau <= 0.0) error->universe_all(FLERR,"Fix path_dynamics tau must be > 0");
            iarg+=1;
	  }
  ...
  while (iarg < narg) {
      if (strcmp(arg[iarg],"parallel") == 0) {
        ...
        ParaSpring = true;
        kspringPara = force->numeric(FLERR,arg[iarg+1]);
        ...
        if (iarg+2 > narg) error->universe_all(FLERR,"Illegal fix neb command");
        climber = force->numeric(FLERR,arg[iarg+1]);
  ...
  global_freq = 1;
  //thermo_energy = 1; //To add spring force to total energy
  vector_flag = 1;
  size_vector = 7;
  extvector   = 1;
  comm_forward = 3;
...
int FixPATH_DYNAMICS::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
  //mask |= THERMO_ENERGY; //To add spring energy to total energy
